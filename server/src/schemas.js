...
const habitFields = {
  ...
  /*
   * Времена напоминаний в сутках человека; пустой список — не напоминать.
   *
   * Список, а не одно время: воду пьют трижды в день, и напомнить о ней надо
   * трижды. Строками, а не числами минут: в этом же виде они приходят из поля
   * ввода, в этом же уходят в текст сообщения, и переводить туда-обратно
   * незачем.
   */
  remindTimes: { type: 'array', maxItems: 5, items: { type: 'string', pattern: TIME_PATTERN } },
}
...
export const reminderSchema = {
  body: {
    ...
    properties: {
      /*
       * Не больше пяти времён на привычку. Не техническое ограничение:
       * шестое сообщение об одной и той же привычке за день человек уже не
       * читает, а идёт выключать бота.
       */
      times: {
        type: 'array',
        maxItems: 5,
