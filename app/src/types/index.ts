...
export interface Habit {
  ...
  /**
   * Норма на день: сколько раз выполнить, чтобы день был закрыт. Вода — три
   * раза, пробежка — один.
   *
  ...
  /**
   * Свои времена напоминаний `ЧЧ:ММ` в сутках человека; пусто — не напоминать.
   *
   * Несколько, потому что воду пьют трижды в день, и напомнить о ней надо
   * трижды. Личные, а не общие на привычку: договорённость — это дни и само
   * дело, а в котором часу кому напомнить, к ней отношения не имеет.
   */
  remindTimes: string[]
  sortOrder: number
  createdAt: string
...
/**
 * Отметка за конкретный день. Строка есть — в этот день привычкой занимались,
 * счётчик говорит сколько раз; строки нет — не занимались вовсе.
 *
 * День считается выполненным, когда счётчик дошёл до нормы привычки. У
 * привычки с нормой в один раз это по-прежнему «строка есть — день выполнен».
 */
